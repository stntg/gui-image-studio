...
class TransformationDemo:
    ...
    def setup_ui(self):
        ...
        title_label = ttk.Label(control_frame, text="Image Transformations", 
                               font=("Arial", 12, "bold"))
        ...
        image_combo = ttk.Combobox(control_frame, textvariable=self.image_var,
                                  values=["colorful.png", "icon.png", "circle.png", "square.png", "triangle.png"],
        ...
        width_scale = Scale(size_frame, from_=32, to=256, orient=tk.HORIZONTAL,
                           variable=self.width_var, command=self.update_image)
        width_scale.grid(row=0, column=1)
        
        ttk.Label(size_frame, text="H:").grid(row=1, column=0)
        height_scale = Scale(size_frame, from_=32, to=256, orient=tk.HORIZONTAL,
                            variable=self.height_var, command=self.update_image)
        height_scale.grid(row=1, column=1)
        
        # Rotation
        ttk.Label(control_frame, text="Rotation (degrees):").pack(anchor=tk.W)
        self.rotation_var = tk.IntVar(value=0)
        rotation_scale = Scale(control_frame, from_=0, to=360, orient=tk.HORIZONTAL,
                              variable=self.rotation_var, command=self.update_image)
        ...
        transparency_scale = Scale(control_frame, from_=0.1, to=1.0, resolution=0.1,
        ...
        contrast_scale = Scale(control_frame, from_=0.1, to=3.0, resolution=0.1,
        ...
        saturation_scale = Scale(control_frame, from_=0.0, to=3.0, resolution=0.1,
        ...
        Scale(tint_frame, from_=0, to=255, orient=tk.HORIZONTAL, variable=self.tint_r,
              command=self.update_image, length=80).grid(row=0, column=1)
        
        ttk.Label(tint_frame, text="G:").grid(row=1, column=0)
        Scale(tint_frame, from_=0, to=255, orient=tk.HORIZONTAL, variable=self.tint_g,
              command=self.update_image, length=80).grid(row=1, column=1)
        
        ttk.Label(tint_frame, text="B:").grid(row=2, column=0)
        Scale(tint_frame, from_=0, to=255, orient=tk.HORIZONTAL, variable=self.tint_b,
              command=self.update_image, length=80).grid(row=2, column=1)
        
        # Tint intensity
        ttk.Label(control_frame, text="Tint Intensity:").pack(anchor=tk.W)
        self.tint_intensity_var = tk.DoubleVar(value=0.0)
        tint_intensity_scale = Scale(control_frame, from_=0.0, to=1.0, resolution=0.1,
                                    orient=tk.HORIZONTAL, variable=self.tint_intensity_var,
                                    command=self.update_image)
        tint_intensity_scale.pack(fill=tk.X, pady=(0, 10))
        
        # Grayscale checkbox
        self.grayscale_var = tk.BooleanVar()
        grayscale_check = ttk.Checkbutton(control_frame, text="Grayscale",
                                         variable=self.grayscale_var,
                                         command=self.update_image)
        grayscale_check.pack(anchor=tk.W, pady=(0, 10))
        
        # Reset button
        reset_button = ttk.Button(control_frame, text="Reset All", command=self.reset_values)
...
def preset_transformations_example():
    ...
    presets = [
        ...
        {
            "name": "Sepia Tone",
            "params": {"size": (64, 64), "tint_color": (255, 200, 150), "tint_intensity": 0.3, "saturation": 0.7}
        },
        {
            "name": "High Contrast",
        ...
        {
            "name": "Blue Tint",
            "params": {"size": (64, 64), "tint_color": (100, 150, 255), "tint_intensity": 0.4}
        },
        {
