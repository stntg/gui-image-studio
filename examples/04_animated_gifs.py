...
class AnimatedGifDemo:
    ...
    def setup_ui(self):
        ...
        # Title
        title_label = ttk.Label(self.root, text="Animated GIF Examples", 
                               font=("Arial", 16, "bold"))
        ...
        # Animation controls
        ttk.Button(control_frame, text="Start All", command=self.start_all_animations).pack(side=tk.LEFT, padx=5)
        ttk.Button(control_frame, text="Stop All", command=self.stop_all_animations).pack(side=tk.LEFT, padx=5)
        ttk.Button(control_frame, text="Reload", command=self.reload_animations).pack(side=tk.LEFT, padx=5)
    ...
    def create_animation_sections(self):
        ...
        ttk.Button(basic_controls, text="Start", 
                  command=lambda: self.start_animation("basic")).pack(side=tk.LEFT, padx=5)
        ttk.Button(basic_controls, text="Stop", 
                  command=lambda: self.stop_animation("basic")).pack(side=tk.LEFT, padx=5)
        
        # Transformed animation section
        transform_frame = ttk.LabelFrame(self.content_frame, text="Transformed Animation")
        ...
        ttk.Button(transform_controls, text="Start All", 
                  command=self.start_transform_animations).pack(side=tk.LEFT, padx=5)
        ttk.Button(transform_controls, text="Stop All", 
        ...
        speed_scale = tk.Scale(speed_controls, from_=0.1, to=3.0, resolution=0.1,
                              orient=tk.HORIZONTAL, variable=self.speed_var,
                              command=self.update_speed)
        speed_scale.pack(side=tk.LEFT, padx=10)
        
        ttk.Button(speed_controls, text="Start", 
                  command=lambda: self.start_speed_animation()).pack(side=tk.LEFT, padx=5)
        ttk.Button(speed_controls, text="Stop", 
            ...
            def make_animate(anim_key, lbl):
                def animate(frame_index=0):
                    if anim_key in self.animation_jobs:
                        ...
                        frame_index = (frame_index + 1) % len(frames)
                        job_id = self.root.after(anim_data["frame_delay"], animate, frame_index)
    ...
    try:
        ...
        try:
            ...
            if frames:
                ...
                start_btn = ctk.CTkButton(button_frame, text="Start", command=start_animation)
                start_btn.pack(side=tk.LEFT, padx=10)
                
                stop_btn = ctk.CTkButton(button_frame, text="Stop", command=stop_animation)
