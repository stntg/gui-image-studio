...
def check_prerequisites():
    ...
    try:
        ...
        print("⚠ CustomTkinter is not installed (optional). Run: pip install customtkinter")
    
    # Check if embedded_images.py exists
    embedded_images_path = os.path.join(os.path.dirname(__file__), '..', 'src', 'embedded_images.py')
...
def run_example(example_file):
    ...
    try:
        ...
        # Run the example
        result = subprocess.run([sys.executable, example_path], cwd=os.path.dirname(__file__))
