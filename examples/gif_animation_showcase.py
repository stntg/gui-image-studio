...
class GifAnimationShowcase:
    ...
    def setup_ui(self):
        ...
        title_label = tk.Label(title_frame, text="GIF Animation Showcase", 
                              font=("Arial", 20, "bold"), bg='#f0f0f0')
        title_label.pack()
        
        subtitle_label = tk.Label(title_frame, text="Comprehensive animated GIF demonstrations", 
    ...
    def create_basic_tab(self):
        ...
        # Animation display
        self.single_label = tk.Label(single_frame, text="Animation will appear here", 
                                    width=20, height=10, relief=tk.SUNKEN)
        ...
        ttk.Button(controls_frame, text="▶ Play", 
                  command=lambda: self.start_animation("single")).pack(side=tk.LEFT, padx=5)
        ttk.Button(controls_frame, text="⏸ Pause", 
                  command=lambda: self.pause_animation("single")).pack(side=tk.LEFT, padx=5)
        ttk.Button(controls_frame, text="⏹ Stop", 
                  command=lambda: self.stop_animation("single")).pack(side=tk.LEFT, padx=5)
        ttk.Button(controls_frame, text="⏮ Reset", 
                  command=lambda: self.reset_animation("single")).pack(side=tk.LEFT, padx=5)
        ...
        self.speed_label = tk.Label(speed_frame, text="Speed controlled animation", 
                                   width=20, height=8, relief=tk.SUNKEN)
        ...
        self.speed_scale = tk.Scale(speed_controls, from_=0.1, to=5.0, resolution=0.1,
        ...
        ttk.Button(speed_buttons, text="Start", 
                  command=lambda: self.start_animation("speed")).pack(side=tk.LEFT, padx=5)
        ttk.Button(speed_buttons, text="Stop", 
                  command=lambda: self.stop_animation("speed")).pack(side=tk.LEFT, padx=5)
    ...
    def create_advanced_tab(self):
        ...
        transform_frame = ttk.LabelFrame(advanced_frame, text="Animation with Transformations")
        ...
        self.transform_configs = [
            ("Original", {"size": (64, 64)}),
            ("Tinted Blue", {"size": (64, 64), "tint_color": (100, 150, 255), "tint_intensity": 0.4}),
            ("High Contrast", {"size": (64, 64), "contrast": 2.0, "saturation": 1.5}),
            ("Grayscale", {"size": (64, 64), "grayscale": True}),
            ("Rotated", {"size": (64, 64), "rotate": 15}),
            ("Scaled", {"size": (96, 96), "tint_color": (255, 200, 100), "tint_intensity": 0.2})
        ]
        ...
        for i, (name, config) in enumerate(self.transform_configs):
            ...
            label = tk.Label(item_frame, text="Loading...", width=12, height=8, relief=tk.SUNKEN)
            label.pack()
        ...
        ttk.Button(transform_controls, text="Start All Transforms", 
                  command=self.start_transform_animations).pack(side=tk.LEFT, padx=5)
        ttk.Button(transform_controls, text="Stop All Transforms", 
                  command=self.stop_transform_animations).pack(side=tk.LEFT, padx=5)
        ...
        self.sync_labels = []
        for i in range(4):
            label = tk.Label(sync_grid, text=f"Sync {i+1}", width=10, height=6, relief=tk.SUNKEN)
        ...
        ttk.Button(sync_controls, text="Start Synchronized", 
                  command=self.start_synchronized_animations).pack(side=tk.LEFT, padx=5)
        ttk.Button(sync_controls, text="Stop Synchronized", 
                  command=self.stop_synchronized_animations).pack(side=tk.LEFT, padx=5)
    ...
    def create_performance_tab(self):
        ...
        perf_scrollbar = ttk.Scrollbar(perf_display_frame, orient=tk.VERTICAL, command=self.perf_text.yview)
        ...
        ttk.Button(perf_controls, text="Start Performance Test", 
                  command=self.start_performance_test).pack(side=tk.LEFT, padx=5)
        ttk.Button(perf_controls, text="Clear Log", 
                  command=self.clear_performance_log).pack(side=tk.LEFT, padx=5)
    ...
    def create_gallery_tab(self):
        ...
        scrollbar = ttk.Scrollbar(gallery_display, orient=tk.VERTICAL, command=canvas.yview)
        ...
        gallery_items = [
            ...
            ("Red Tint", "animation.gif", {"tint_color": (255, 100, 100), "tint_intensity": 0.5}),
            ("Green Tint", "animation.gif", {"tint_color": (100, 255, 100), "tint_intensity": 0.5}),
            ("Blue Tint", "animation.gif", {"tint_color": (100, 100, 255), "tint_intensity": 0.5}),
        ...
        for i, (name, filename, config) in enumerate(gallery_items):
            ...
            label = tk.Label(item_frame, text="Loading...", width=10, height=8, relief=tk.SUNKEN)
            label.pack()
            
            name_label = ttk.Label(item_frame, text=name, font=("Arial", 8))
            name_label.pack()
            
            self.gallery_labels[name] = {"label": label, "filename": filename, "config": config}
        ...
        ttk.Button(gallery_controls, text="Start All Gallery", 
                  command=self.start_gallery_animations).pack(side=tk.LEFT, padx=5)
        ttk.Button(gallery_controls, text="Stop All Gallery", 
                  command=self.stop_gallery_animations).pack(side=tk.LEFT, padx=5)
    ...
    def create_global_controls(self):
        ...
        ttk.Button(global_frame, text="🎬 Start All Animations", 
                  command=self.start_all_animations).pack(side=tk.LEFT, padx=5)
        ttk.Button(global_frame, text="⏹ Stop All Animations", 
                  command=self.stop_all_animations).pack(side=tk.LEFT, padx=5)
        ttk.Button(global_frame, text="🔄 Reload All", 
                  command=self.reload_all_animations).pack(side=tk.LEFT, padx=5)
    ...
    def load_animations(self):
        ...
        try:
            ...
            self.log_performance("Animation Loading", f"Successfully loaded {len(self.animations)} animations")
    ...
    def performance_test_results(self):
        ...
        self.log_performance("Performance Test Results", 
                           f"Total frames rendered: {total_frames}")
        self.log_performance("Performance Test Results", 
                           f"Active animations: {active_animations}")
        self.log_performance("Performance Test Results", 
