...
class AdvancedFeaturesDemo:
    ...
    def create_format_tab(self, notebook):
        ...
        ttk.Label(format_frame, text="Format Conversion Examples", 
        ...
        format_combo = ttk.Combobox(control_frame, textvariable=self.format_var,
                                   values=["PNG", "JPEG", "BMP", "TIFF"], state="readonly")
        format_combo.pack(side=tk.LEFT, padx=10)
        
        convert_btn = ttk.Button(control_frame, text="Convert & Display", 
        ...
        self.format_image_label = tk.Label(self.format_display_frame, text="Converted image will appear here")
    ...
    def create_error_handling_tab(self, notebook):
        ...
        ttk.Label(error_frame, text="Error Handling Examples", 
                 font=("Arial", 12, "bold")).pack(pady=10)
        ...
        for i, (description, image_name) in enumerate(scenarios):
            ...
            if i == 0:  # Non-existent image
                ttk.Button(btn_frame, text="Try to load non-existent image",
                          command=lambda: self.test_nonexistent_image()).pack()
            elif i == 1:  # Invalid theme
                ttk.Button(btn_frame, text="Try invalid theme",
                          command=lambda: self.test_invalid_theme()).pack()
            elif i == 2:  # Invalid parameters
                ttk.Button(btn_frame, text="Try invalid parameters",
                          command=lambda: self.test_invalid_parameters()).pack()
        ...
        # Clear log button
        ttk.Button(error_frame, text="Clear Log", command=self.clear_error_log).pack(pady=5)
    ...
    def create_performance_tab(self, notebook):
        ...
        ttk.Label(perf_frame, text="Performance Testing", 
                 font=("Arial", 12, "bold")).pack(pady=10)
        ...
        ttk.Button(test_frame, text="Batch Load Test", 
                  command=self.run_batch_load_test).pack(side=tk.LEFT, padx=5)
        ttk.Button(test_frame, text="Transformation Speed Test", 
                  command=self.run_transformation_test).pack(side=tk.LEFT, padx=5)
        ttk.Button(test_frame, text="Memory Usage Test", 
                  command=self.run_memory_test).pack(side=tk.LEFT, padx=5)
        
        # Progress bar
        self.progress_var = tk.DoubleVar()
        self.progress_bar = ttk.Progressbar(perf_frame, variable=self.progress_var, maximum=100)
    ...
    def create_complex_transformations_tab(self, notebook):
        ...
        ttk.Label(complex_frame, text="Complex Transformation Combinations", 
                 font=("Arial", 12, "bold")).pack(pady=10)
        ...
        for i, preset in enumerate(presets):
            ...
            # Apply button
            ttk.Button(preset_frame, text="Apply Transformation",
                      command=lambda p=preset: self.apply_complex_transformation(p)).pack(pady=5)
    ...
    def create_integration_tab(self, notebook):
        ...
        ttk.Label(integration_frame, text="Integration Patterns & Best Practices", 
                 font=("Arial", 12, "bold")).pack(pady=10)
...
7. CONFIGURATION-DRIVEN LOADING
   ...
   def load_configured_image(image_name, config_key, **overrides):
       ...
       return gui_image_studio.get_image(image_name, **config)
        ...
        # Demo button
        ttk.Button(integration_frame, text="Run Integration Demo", 
                  command=self.run_integration_demo).pack(pady=10)
    ...
    def run_batch_load_test(self):
        ...
        def test():
            ...
            images = ["icon.png", "colorful.png", "circle.png", "square.png", "triangle.png"]
                ...
                for size in sizes:
                    try:
                        ...
                        self.perf_results.insert(tk.END, 
                            f"Loaded {image_name} at {size}: {load_time:.3f}s\n")
                        
                    except Exception as e:
                        self.perf_results.insert(tk.END, 
                            f"Failed to load {image_name} at {size}: {e}\n")
            ...
            self.perf_results.insert(tk.END, 
                f"Average time per image: {total_time/total_tests:.3f}s\n\n")
    ...
    def run_transformation_test(self):
        ...
        def test():
            ...
            transformations = [
                ...
                ("Complex", {"grayscale": True, "rotate": 30, "contrast": 1.5, "saturation": 0.5})
            ]
            ...
            for i, (name, params) in enumerate(transformations):
                try:
                    ...
                    self.perf_results.insert(tk.END, 
                        f"{name} transformation: {load_time:.3f}s\n")
                    
                except Exception as e:
                    self.perf_results.insert(tk.END, 
                        f"{name} transformation failed: {e}\n")
    ...
    def run_memory_test(self):
        ...
        self.perf_results.insert(tk.END, "Memory test: Loading multiple large images...\n")
        ...
        try:
            ...
            self.perf_results.insert(tk.END, 
                f"Successfully loaded {len(images)} large images\n")
            self.perf_results.insert(tk.END, 
    ...
    def run_integration_demo(self):
        ...
        def load_cached_image():
            try:
                ...
                image = manager.get_image("icon.png", framework="tkinter", size=(64, 64))
        ...
        ttk.Button(demo_window, text="Load Image (Cached)", 
                  command=load_cached_image).pack(pady=5)
        
        ttk.Label(demo_window, text="Click multiple times - first load is slower,\nsubsequent loads use cache").pack(pady=10)
