    ...
    def load_themed_images(self):
        ...
        for image_name, description in images_to_load:
            try:
                ...
                # Description label
                desc_label = ttk.Label(img_frame, text=f"{description}\n({theme} theme)")
                desc_label.pack()
    ...
    def add_theme_comparison(self):
        ...
        for i, theme in enumerate(themes):
            try:
                ...
                placeholder = tk.Label(theme_frame, text="No image\navailable", 
                                     width=10, height=3, relief=tk.SUNKEN)
                placeholder.pack()
...
def customtkinter_theming_example():
    ...
    try:
        ...
        def switch_theme():
            ...
            theme_button.configure(text=f"Switch to {'Dark' if new_mode == 'light' else 'Light'} Mode")
        ...
        def switch_image_theme():
            ...
            image_theme_button.configure(text=f"Image Theme: {current_image_theme['value'].title()}")
        ...
        def load_themed_images(theme):
            ...
            for i, (image_name, description) in enumerate(images_to_load):
                try:
                    ...
                    # Description label
                    desc_label = ctk.CTkLabel(img_container, text=f"{description}\n({theme})")
                    desc_label.pack(pady=5)
                    
                    image_labels[image_name] = img_label
                    
                except ValueError as e:
                    print(f"Warning: Could not load {image_name} with theme '{theme}': {e}")
                    ...
                    try:
                        ...
                        placeholder = ctk.CTkLabel(img_container, text="Image\nNot Available", 
                                                 width=64, height=64)
        ...
        # UI Setup
        title_label = ctk.CTkLabel(root, text="CustomTkinter Theming Demo", 
        ...
        # Status label
        status_label = ctk.CTkLabel(root, text=f"Current appearance: {initial_mode.title()}")
