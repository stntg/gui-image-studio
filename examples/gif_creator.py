...
def create_rotating_square(filepath, size=(64, 64), frames=24, color=(100, 255, 100)):
    ...
    for frame in range(frames):
        ...
        corners = []
        for dx, dy in [(-square_size, -square_size), (square_size, -square_size), 
        ...
        # Add center dot
        dot_size = 3
        draw.ellipse([center[0] - dot_size, center[1] - dot_size,
                     center[0] + dot_size, center[1] + dot_size],
...
def create_progress_bar(filepath, size=(128, 32), frames=20, color=(0, 200, 0)):
    ...
    for frame in range(frames):
        ...
        draw.rounded_rectangle([2, 2, size[0] - 2, size[1] - 2], 
                              radius=5, fill=bg_color, outline=(100, 100, 100, 255))
        ...
        if fill_width > 0:
            draw.rounded_rectangle([4, 4, 4 + fill_width, size[1] - 4], 
                                  radius=3, fill=color + (255,))
        
        # Add shine effect
        if fill_width > 10:
            shine_x = 4 + fill_width - 10
            draw.rounded_rectangle([shine_x, 6, shine_x + 8, size[1] - 6], 
                                  radius=2, fill=(255, 255, 255, 100))
...
def create_bouncing_ball(filepath, size=(64, 64), frames=30, color=(255, 150, 0)):
    ...
    for frame in range(frames):
        ...
        t = frame / frames
        # Parabolic motion
        y = size[1] - ball_radius - 5 - abs(math.sin(t * 2 * math.pi)) * (size[1] - 2 * ball_radius - 10)
        x = size[0] // 2
        ...
        if shadow_alpha > 0:
            draw.ellipse([x - ball_radius, shadow_y - 2, x + ball_radius, shadow_y + 2],
                        fill=(0, 0, 0, shadow_alpha))
        
        # Ball
        draw.ellipse([x - ball_radius, y - ball_radius, x + ball_radius, y + ball_radius],
                    fill=color + (255,), outline=(200, 100, 0, 255), width=1)
        
        # Ball highlight
        highlight_size = 3
        draw.ellipse([x - highlight_size, y - ball_radius + 2, 
                     x + highlight_size, y - ball_radius + 2 + highlight_size * 2],
...
def create_all_sample_gifs():
    ...
    create_loading_spinner(os.path.join(output_dir, "spinner_dark.gif"), 
                          colors=[(150, 150, 150), (200, 200, 200), (255, 255, 255)])
    create_pulse_animation(os.path.join(output_dir, "pulse_green.gif"), 
                          color=(100, 255, 100))
    create_pulse_animation(os.path.join(output_dir, "pulse_blue.gif"), 
                          color=(100, 150, 255))
