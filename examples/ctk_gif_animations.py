...
try:
    ...
    CTK_AVAILABLE = False
    print("CustomTkinter not available. Please install it with: pip install customtkinter")
...
class CTkGifAnimationDemo:
    ...
    def setup_ui(self):
        ...
        # Main title
        title_label = ctk.CTkLabel(self.root, text="CustomTkinter GIF Animations", 
    ...
    def setup_loading_tab(self):
        ...
        ctk.CTkLabel(spinner_frame, text="Loading Spinners", 
        ...
        ctk.CTkButton(spinner_controls, text="Start Spinners", 
                     command=self.start_spinners).pack(side="left", padx=5)
        ctk.CTkButton(spinner_controls, text="Stop Spinners", 
                     command=self.stop_spinners).pack(side="left", padx=5)
        
        # Progress indicators
        progress_frame = ctk.CTkFrame(tab)
        progress_frame.pack(fill="x", padx=20, pady=10)
        
        ctk.CTkLabel(progress_frame, text="Animated Progress Indicators", 
                    font=ctk.CTkFont(size=16, weight="bold")).pack(pady=10)
        ...
        ctk.CTkButton(progress_controls, text="Start Progress", 
                     command=self.start_progress_animation).pack(side="left", padx=5)
        ctk.CTkButton(progress_controls, text="Stop Progress", 
                     command=self.stop_progress_animation).pack(side="left", padx=5)
    ...
    def setup_interactive_tab(self):
        ...
        ctk.CTkLabel(button_frame, text="Animated Buttons", 
                    font=ctk.CTkFont(size=16, weight="bold")).pack(pady=10)
        ...
        for i, btn_type in enumerate(button_types):
            ...
            # Button
            button = ctk.CTkButton(btn_frame, text=btn_type, width=80,
        ...
        ctk.CTkLabel(hover_frame, text="Hover Animations", 
                    font=ctk.CTkFont(size=16, weight="bold")).pack(pady=10)
    ...
    def setup_themed_tab(self):
        ...
        ctk.CTkLabel(theme_selector_frame, text="Theme Selection", 
                    font=ctk.CTkFont(size=16, weight="bold")).pack(pady=10)
        ...
        ctk.CTkRadioButton(theme_controls, text="Dark Theme", variable=self.theme_var, 
                          value="dark", command=self.change_theme).pack(side="left", padx=10)
        ctk.CTkRadioButton(theme_controls, text="Light Theme", variable=self.theme_var, 
                          value="light", command=self.change_theme).pack(side="left", padx=10)
        
        # Themed animations display
        themed_frame = ctk.CTkFrame(tab)
        themed_frame.pack(fill="both", expand=True, padx=20, pady=10)
        
        ctk.CTkLabel(themed_frame, text="Themed Animations", 
                    font=ctk.CTkFont(size=16, weight="bold")).pack(pady=10)
        ...
        ctk.CTkButton(themed_controls, text="Start Themed", 
                     command=self.start_themed_animations).pack(side="left", padx=5)
        ctk.CTkButton(themed_controls, text="Stop Themed", 
                     command=self.stop_themed_animations).pack(side="left", padx=5)
    ...
    def setup_advanced_tab(self):
        ...
        ctk.CTkLabel(perf_frame, text="Performance Monitor", 
                    font=ctk.CTkFont(size=16, weight="bold")).pack(pady=10)
        ...
        ctk.CTkButton(perf_controls, text="Start Monitoring", 
                     command=self.start_performance_monitoring).pack(side="left", padx=5)
        ctk.CTkButton(perf_controls, text="Clear Log", 
                     command=self.clear_performance_log).pack(side="left", padx=5)
        
        # Advanced animation controls
        advanced_frame = ctk.CTkFrame(tab)
        advanced_frame.pack(fill="both", expand=True, padx=20, pady=10)
        
        ctk.CTkLabel(advanced_frame, text="Advanced Animation Controls", 
                    font=ctk.CTkFont(size=16, weight="bold")).pack(pady=10)
        ...
        self.speed_slider = ctk.CTkSlider(speed_frame, from_=0.1, to=3.0, 
                                         variable=self.speed_var, command=self.update_global_speed)
        ...
        ctk.CTkLabel(quality_frame, text="Animation Quality:").pack(side="left", padx=10)
        
        self.quality_var = ctk.StringVar(value="high")
        quality_menu = ctk.CTkOptionMenu(quality_frame, values=["low", "medium", "high"], 
    ...
    def setup_global_controls(self):
        ...
        ctk.CTkButton(control_frame, text="🎬 Start All", 
                     command=self.start_all_animations).pack(side="left", padx=5)
        ctk.CTkButton(control_frame, text="⏹ Stop All", 
                     command=self.stop_all_animations).pack(side="left", padx=5)
        ctk.CTkButton(control_frame, text="🔄 Reload", 
                     command=self.reload_animations).pack(side="left", padx=5)
    ...
    def load_animations(self):
        ...
        try:
            ...
            self.status_label.configure(text=f"Loaded {len(self.animations)} animations")
            self.log_performance(f"Successfully loaded {len(self.animations)} animations")
    ...
    def toggle_button_animation(self, button_type):
        ...
        if self.animation_states.get(animation_key, False):
            self.stop_animation(animation_key)
            self.animated_buttons[button_type]["button"].configure(text=f"Start {button_type}")
        else:
            label = self.animated_buttons[button_type]["icon"]
            self.start_animation(animation_key, label)
            self.animated_buttons[button_type]["button"].configure(text=f"Stop {button_type}")
