...
The text tool settings (font size, family, bold, italic) were not being applied when
users clicked on the canvas to add text.

## Root Cause

The issue was in the canvas click handling in `canvas_manager.py`. When the text
tool was selected and the user clicked on the canvas, it was calling an old
`add_text()` method that bypassed the entire drawing tools system and used
...
- **Arial**: Uses `arialbd.ttf` for bold, `ariali.ttf` for italic,
...
- `src/gui_image_studio/image_studio/core/canvas_manager.py`
  Fixed canvas click handling
- `src/gui_image_studio/image_studio/toolkit/tools/text_tool.py`
  Enhanced font loading

## Status

🎉 **RESOLVED** - Text tool now fully respects all user settings and properly
renders text with the chosen font size, family, bold, and italic formatting.
