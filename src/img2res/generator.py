...
def embed_images_from_folder(folder_path, output_file="embedded_images.py", compression_quality=85):
    ...
    if not os.path.exists(folder_path):
        print(f"Warning: Folder '{folder_path}' does not exist. Creating empty embedded_images.py")
        images_dict = {"default": {}}
    else:
...
def generate_embedded_images():
    ...
    parser = argparse.ArgumentParser(description='Generate embedded images from a folder')
